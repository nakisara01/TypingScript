...
### 작업 내용
- LessonPlayer의 정답 비교 로직에 `codesMatch` 함수를 도입해 줄별 트림 및 줄바꿈 정규화를 거친 뒤 비교하도록 수정, CSS 등 줄 들여쓰기 차이로 완료 판정이 막히던 문제를 해결

### 변경 이유
- 사용자가 코드 내용을 올바르게 입력했는데도 공백·들여쓰기 차이 때문에 레슨을 완료하지 못하는 오류를 방지하기 위함
...
### 확인 방법
- `npm run dev` 실행 후 `/language/css`에서 제공된 코드와 동일한 내용을 입력한 뒤 들여쓰기를 다르게 두더라도 레슨 완료 및 Next 버튼이 나타나는지 확인

### 남은 과제
- 공백 외의 다른 정규화 규칙(예: 따옴표 변형, 속성 순서 등)에 대한 허용 범위를 정의하고, 향후 레슨별 커스텀 채점 규칙을 적용할 방안을 검토

### 다음 작업 제안
- 코드 비교 로직을 테스트로 검증하거나, 완료 판정에 실패했을 때 Diff 뷰를 자동으로 펼쳐 바로 차이점을 확인하도록 UX를 개선
